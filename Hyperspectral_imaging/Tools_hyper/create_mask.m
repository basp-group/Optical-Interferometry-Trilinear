function [ M1, M2, M3,count,M_p,M_s_large,M_s_zero] = ...
    create_mask(T,n,m,M_b,u_hf,seed,c,plots)

% c : vector position of 0 freq
% u_hf : low freq. prop. in bispectrum
count = 0;
...
M_hf = floor(u_hf*n*m) ;
if M_hf < 2*M_p
mat = gaussian_circle(n,m,1/30) ;
mat_f = find(mat);
mask_ind_hf = intersect(mat_f, mask_ind) ;
mask_hf = zeros(n*m,1) ;
