...
L1_ps,L2_ps,L3_ps, L1_bs,L2_bs,L3_bs,L0] = L1_L2_L3_completeF(M,M_p,Mb,L)

% This function generates selection operators to select frequency
% coefficients from u1, u2, u3 to construct power spectrum and bispectrum
% measurements
...
L3 = zeros(M,1);
L3(1:M_p) = M_p+1; %corresponds to zero frequency
...
% powerspectrum
L_p = zeros(M_p, 2*M_p+1) ;
L_p(:, 1:M_p) = eye(M_p) ;
L_p_0freq = zeros(size(L_p)) ;
L_p_0freq(:, 1:M_p+1) = L3_m(1:M_p, 1:M_p+1) ;
L_conj = zeros(size(L_p)) ;
L_conj(:,M_p+2:end) = fliplr(eye(M_p)) ;
